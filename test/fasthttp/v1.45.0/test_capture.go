...
func setupCaptureHTTP(ctx *fasthttp.RequestCtx) {
	if string(ctx.PostBody()) != captureRequestBody {
		log.Fatalf("server request body = %q, want %q", ctx.PostBody(), captureRequestBody)
	}
	ctx.Response.Header.SetContentType("application/json")
	ctx.Response.SetBodyString(captureResponseBody)
}

func main() {
	go func() {
		if err := fasthttp.ListenAndServe(":8080", setupCaptureHTTP); err != nil {
			log.Fatal(err)
		}
	}()
	...
	req.SetRequestURI("http://localhost:8080/capture")
