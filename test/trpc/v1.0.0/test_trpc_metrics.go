...
func main() {
	...
	verifier.WaitAndAssertMetrics(map[string]func(metricdata.ResourceMetrics){
		"rpc.server.duration": func(mrs metricdata.ResourceMetrics) {
			...
			verifier.VerifyRpcServerMetricsAttributes(point.DataPoints[0].Attributes.ToSlice(), "SayHello", "Greeter", "trpc", "127.0.0.1:8000")
		},
		"rpc.client.duration": func(rm metricdata.ResourceMetrics) {
			...
			verifier.VerifyRpcClientMetricsAttributes(point.DataPoints[0].Attributes.ToSlice(), "", "service", "trpc", "127.0.0.1:8000")
		},
	})
}
