...
type streamReader[T streamable] struct {
	...
	OtelResponseChan chan interface{}
}

func (stream *streamReader[T]) Recv() (response T, err error) {
	defer func() {
		if err != nil {
			close(stream.OtelResponseChan)
		}
	}()
	...
	stream.OtelResponseChan <- response
	return response, nil
}

func (stream *streamReader[T]) GetOtelEndChan() chan interface{} {
	return stream.OtelResponseChan
}

func (stream *streamReader[T]) InitOtelChan() {
	stream.OtelResponseChan = make(chan interface{}, 10)
}
