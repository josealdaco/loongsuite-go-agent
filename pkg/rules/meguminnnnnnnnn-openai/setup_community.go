...
//go:linkname communityCreateChatCompletionOnEnter github.com/meguminnnnnnnnn/go-openai.communityCreateChatCompletionOnEnter
func communityCreateChatCompletionOnEnter(call api.CallContext, client *openai.Client, ctx context.Context, request openai.ChatCompletionRequest, opts ...openai.ChatCompletionRequestOption) {
	...
	req := openaiRequest{
		...
		temperature:      float64PtrToFloat64(request.Temperature),
...
//go:linkname communityCreateChatCompletionOnExit github.com/meguminnnnnnnnn/go-openai.communityCreateChatCompletionOnExit
...
//go:linkname communityCreateChatCompletionStreamOnEnter github.com/meguminnnnnnnnn/go-openai.communityCreateChatCompletionStreamOnEnter
func communityCreateChatCompletionStreamOnEnter(call api.CallContext, client *openai.Client, ctx context.Context, request openai.ChatCompletionRequest, opts ...openai.ChatCompletionRequestOption) {
	if !openaiEnabler.Enable() {
		return
	}
...
//go:linkname communityCreateChatCompletionStreamOnExit github.com/meguminnnnnnnnn/go-openai.communityCreateChatCompletionStreamOnExit
	...
	go func() {
		...
		for {
			...
			case resp, ok := <-stream.GetOtelEndChan():
				...
				resp1, ok1 := resp.(openai.ChatCompletionStreamResponse)
				if ok1 {
...
func float64PtrToFloat64(p *float32) float64 {
	if p == nil {
		return 0
	}
	return float64(*p)
