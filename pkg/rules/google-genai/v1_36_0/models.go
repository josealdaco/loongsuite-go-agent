...
func (m Models) generateContentStream(ctx context.Context, model string, contents []*Content, config *GenerateContentConfig) iter.Seq2[*GenerateContentResponse, error] {
	...
	respChan, ok := ctx.Value("arms_resp_chan").(chan SpanResp)
	body, err := toConverter(m.apiClient, parameterMap, nil)
	if err != nil {
		if ok {
			respChan <- SpanResp{nil, err}
			close(respChan)
		}
	...
	streamIter := iterateResponseStream(&rs, func(responseMap map[string]any) (*GenerateContentResponse, error) {
		responseMap, err := fromConverter(responseMap, nil)
		if err != nil {
			if ok {
				respChan <- SpanResp{nil, err}
		...
		if ok {
			respChan <- SpanResp{response, nil}
		}
		return response, nil
	})
	...
	return func(yield func(*GenerateContentResponse, error) bool) {
		...
		streamIter(func(resp *GenerateContentResponse, err error) bool {
